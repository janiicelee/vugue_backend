    ...
    def get(self, request, category_id):
        try:
            ...
            if 'tag_id' in request.GET:
                ...
                articles = CategoryTagArticle.objects.select_related('article','category','tag').filter(category_id = category_id, tag_id = tag_id).order_by('id')[offset:limit]
                data     = self._article_data(articles)
                return JsonResponse({'data':data}, status=200)

            articles = CategoryTagArticle.objects.select_related('article','category').filter(category_id = category_id).order_by('id')[offset*limit:(offset+1)*limit]
            data     = self._article_data(articles)
...
class SearchView(View):
    def get(self, request):
        if 'keyword' in request.GET:
            ...
            articles = Article.objects.filter(Q(title__icontains=keyword)|Q(caption__icontains=keyword))
            article_list = [{
                'title'     : article.title,
...
class DetailView(View):
    ...
    def get(self, request, detail_id):
        try:
            ...
            detail      = ArticleDetail.objects.get(id=detail_id)
            detail_info = [{
                'title'        : detail.title,
            ...
            articles = CategoryTagArticle.objects.select_related('article').filter(category_id = category_id).order_by('id')[0:limit]
            article_list = [{
                'title'        : props.article.title,
...
class MainView(View):
    ...
    def get(self, request):
        try:
            ...
            main_articles = CategoryTagArticle.objects.select_related('article','tag').filter(tag_id = self.MAIN_TAG).order_by('id')[0:limit]
            article_list  = [{
                'title'     : props.article.title,
                'image_url' : props.article.image_url,
                'detail_id' : props.article.article_detail_id
